...
This sequence is for **`x-*` admin routes** (for example `x-admin.{HOST}`). Public psinode routes (`{HOST}` and non-`x-*` `*.{HOST}`) do not run Authelia forward-auth; Traefik removes only the `authelia_session` cookie before psinode sees the request. Other cookies still reach psinode, even though the browser may still send the `HOST`-scoped session cookie to Traefik.
